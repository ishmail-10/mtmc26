 ...
 */
    ...
    function setEventDateMode(mode) {
      ...
      if (mode === 'single') {
        if (btnSingle) btnSingle.className = 'px-2.5 py-1 rounded font-bold transition bg-cyan-500 text-white shadow-sm';
        if (btnRange) btnRange.className = 'px-2.5 py-1 rounded font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition';
        if (contSingle) contSingle.classList.remove('hidden');
        if (contRange) contRange.classList.add('hidden');
      } else {
        if (btnRange) btnRange.className = 'px-2.5 py-1 rounded font-bold transition bg-cyan-500 text-white shadow-sm';
        if (btnSingle) btnSingle.className = 'px-2.5 py-1 rounded font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition';
    ...
    function openEventViewer(eventId) {
      ...
      if (dateEl) dateEl.innerHTML = `<i data-lucide="calendar" class="w-3.5 h-3.5 text-cyan-400"></i><span>${escapeHtml(ev.date || '')}</span>`;
      ...
      if (badgeEl) {
        ...
        badgeEl.className = `text-xs font-bold px-2.5 py-0.5 rounded border ${categoryColors[ev.category] || 'text-cyan-400 bg-cyan-500/10 border-cyan-500/30'}`;
      }
      ...
      if (modControls) {
        ...
        if (isModOrAdmin || isCreator) {
          modControls.innerHTML = `
            <button type="button" onclick="deleteBatchEvent('${ev.id}')" class="px-3 py-1.5 rounded bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/60 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-xs font-bold transition flex items-center gap-1.5 shadow-sm">
      ...
      // Default view mode: 100% Free Native Google Drive Embed (Zero API keys, Zero Billing)
    ...
    function setEventViewMode(mode) {
      ...
      if (mode === 'collage') {
        ...
        if (collageBtn) {
          collageBtn.className = 'px-3 py-1.5 rounded font-bold bg-cyan-600 text-white transition flex items-center gap-1.5 shadow-sm';
        }
        if (driveBtn) {
          driveBtn.className = 'px-3 py-1.5 rounded font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition flex items-center gap-1.5';
        }
      } else {
        if (collageContainer) collageContainer.classList.add('hidden');
        if (driveContainer) driveContainer.classList.remove('hidden');
        if (driveBtn) {
          driveBtn.className = 'px-3 py-1.5 rounded font-bold bg-cyan-600 text-white transition flex items-center gap-1.5 shadow-sm';
        }
        if (collageBtn) {
          collageBtn.className = 'px-3 py-1.5 rounded font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition flex items-center gap-1.5';
        }
      }
    }
    ...
    async function fetchDriveFolderPhotos(folderId) {
      ...
      try {
        ...
        grid.innerHTML = files.map(f => {
          ...
          return `
            <div class="relative group aspect-square rounded-md overflow-hidden bg-slate-900 border border-slate-700/80 hover:border-cyan-400 cursor-pointer transition shadow hover:shadow-cyan-500/20" onclick="openLightbox('${direct4kUrl}', '${escapeHtml(f.name)}', '📸 4K Ultra-HD · Google Drive')">
              ...
              <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <span class="text-[10px] font-bold text-white px-2 py-1 rounded bg-black/70 backdrop-blur-sm border border-white/20 flex items-center gap-1 shadow">
    ...
    function renderEventsGallery() {
      ...
      let filterPillsHtml = categoryBadges.map(b => {
        ...
        return `
          <button onclick="filterEventCategory('${b.key}')" class="px-3 py-1.5 rounded text-xs font-semibold transition whitespace-nowrap ${isActive ? 'bg-cyan-500 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-900/80 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800'}">
            ${b.label}
          </button>
        `;
      }).join('');
      ...
      if (filtered.length === 0) {
        cardsHtml = `
          <div class="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-md p-6 text-center space-y-2.5">
            ...
            <button onclick="openEventModal()" class="px-3.5 py-1.5 rounded bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold transition inline-flex items-center gap-1.5">
        ...
        cardsHtml = `
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ${filtered.map(ev => {
              ...
              return `
                <div class="bg-gradient-to-br from-slate-900/95 via-[#0b101e] to-slate-950 border border-slate-800/80 hover:border-cyan-500/50 rounded-md overflow-hidden shadow-xl transition-all duration-300 flex flex-col group cursor-pointer" onclick="openEventViewer('${ev.id}')">
                  ...
                  <div class="relative aspect-video w-full bg-slate-950 overflow-hidden">
                    ...
                    <div class="absolute top-3 left-3 flex items-center gap-1.5 pointer-events-none">
                      <span class="text-[10px] font-bold px-2.5 py-0.5 rounded border backdrop-blur-md ${catColor}">
                    ...
                    <div class="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs pointer-events-none">
                      <span class="font-bold flex items-center gap-1 text-[11px] drop-shadow-md text-amber-300">
                        <i data-lucide="calendar" class="w-3.5 h-3.5 text-amber-400"></i> ${escapeHtml(ev.date || '')}
                      </span>
                      <span class="text-[10px] px-2 py-0.5 rounded bg-black/60 text-slate-200 border border-white/20 backdrop-blur-sm flex items-center gap-1">
      ...
      container.innerHTML = `
        ...
        <div class="bg-gradient-to-br from-slate-900 via-[#0a0f1d] to-slate-950 border border-slate-800/80 rounded-md p-5 sm:p-6 relative overflow-hidden shadow-xl text-white">
          ...
          <div class="relative z-10 space-y-3">
            <div class="flex items-center justify-between flex-wrap gap-3">
              <span class="px-2.5 py-1 rounded bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
                <span class="w-2 h-2 rounded-full bg-cyan-400 animate-ping"></span>
                <span>Batch Moments & Events</span>
              </span>

              <button onclick="openEventModal()" class="px-3.5 py-1.5 rounded bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-semibold transition shadow-md inline-flex items-center gap-1.5">
                <i data-lucide="plus" class="w-3.5 h-3.5"></i>
